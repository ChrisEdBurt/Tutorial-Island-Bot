"""
Contains non-skilling player behaviours.
...
def login_basic(username_file=startup.config.get('main', 'username_file'),
                password_file=startup.config.get('main', 'password_file'),
    ...
    for _ in range(3):
        ...
        if existing_user_button is True or ok_button is True:
            ...
            if credential_screen is True:
                ...
                input.Mouse(ltwh=(vision.login_field_left, vision.login_field_top,
                                  startup.LOGIN_FIELD_WIDTH, startup.LOGIN_FIELD_HEIGHT)).click_coord()
                ...
                input.Mouse(ltwh=(vision.pass_field_left, vision.pass_field_top,
                                  startup.LOGIN_FIELD_WIDTH, startup.LOGIN_FIELD_HEIGHT)).click_coord()
    ...
    for _ in range(3):
        ...
        if postlogin_screen_button is True:
            ...
            if logged_in is True:
                ...
                startup.startup_time = time.time()
...
def logout_break_range():
    ...
    if current_time >= startup.checkpoint_1 and startup.checkpoint_1_checked is False:
        log.info('Rolling for checkpoint 1...')
        startup.checkpoint_1_checked = True
        logout_break_roll(5)

    elif current_time >= startup.checkpoint_2 and startup.checkpoint_2_checked is False:
        log.info('Rolling for checkpoint 2...')
        startup.checkpoint_2_checked = True
        logout_break_roll(5)

    elif current_time >= startup.checkpoint_3 and startup.checkpoint_3_checked is False:
        log.info('Rolling for checkpoint 3...')
        startup.checkpoint_3_checked = True
        logout_break_roll(5)

    elif current_time >= startup.checkpoint_4 and startup.checkpoint_4_checked is False:
        log.info('Rolling checkpoint 4...')
        startup.checkpoint_4_checked = True
        logout_break_roll(5)
    ...
    elif current_time >= startup.checkpoint_5:
        startup.checkpoint_1_checked = False
        startup.checkpoint_2_checked = False
        startup.checkpoint_3_checked = False
        startup.checkpoint_4_checked = False
        logout_break_roll(1)

    # Print the correct logging information according to which checkpoint(s)
    #   have been rolled for.
    else:
        if startup.checkpoint_1_checked is False:
            log.info('Checkpoint 1 is at %s', time.ctime(startup.checkpoint_1))
        elif startup.checkpoint_1_checked is True and startup.checkpoint_2_checked is False:
            log.info('Checkpoint 2 is at %s', time.ctime(startup.checkpoint_2))
        elif startup.checkpoint_2_checked is True and startup.checkpoint_3_checked is False:
            log.info('Checkpoint 3 is at %s', time.ctime(startup.checkpoint_3))
        elif startup.checkpoint_3_checked is True and startup.checkpoint_4_checked is False:
            log.info('Checkpoint 4 is at %s', time.ctime(startup.checkpoint_4))
        elif startup.checkpoint_4_checked is True:
            log.info('Checkpoint 5 is at %s', time.ctime(startup.checkpoint_5))
    return True


def logout_break_roll(chance,
                      wait_min=int(startup.config.get('main', 'min_break_duration')),
                      wait_max=int(startup.config.get('main', 'max_break_duration'))):
    """
    ...
    if logout_roll == chance:
        ...
        startup.session_num += 1
        log.info('Completed session %s/%s', startup.session_num, startup.session_total)

        # If the maximum number of sessions has been reached, kill the bot.
        if startup.session_num >= startup.session_total:
...
def check_skills():
    """
    Used to mimic human-like behaviour. Checks the stats of a random
    skill.
    ...
    if config == 'quantity':
        if status == 'all':
            ...
            if quantity_all_enabled is False:
                quantity_all_disabled = vision.Vision(ltwh)
                return
